...
from sleekxmpp.plugins.ieee_sensordata.timerreset import TimerReset
...
from sleekxmpp.plugins.ieee_sensordata.stanza import Sensordata

log = logging.getLogger(__name__)


class IEEE_SENSORDATA(BasePlugin):
    ...
    name = 'ieee_SensorData'
    description = 'ieee_SensorData Internet of Things - Sensor Data'
    ...
    def plugin_init(self):
        """ Start the IEEE_SensorData plugin """
    ...
    def plugin_end(self):
        """ Stop the IEEE_SensorData plugin """
        self.sessions.clear()
    ...
    def _handle_event_req(self, iq):
        ...
        if req_ok:
            ...
            if self.threaded:
                ...
                tr_req = Thread(target=self._threaded_node_request, args=(
                    session, process_fields, req_flags))
                tr_req.start()
    ...
    def _event_delayed_req(self, session, process_fields, req_flags):
        ...
        if self.threaded:
            tr_req = Thread(target=self._threaded_node_request,
                            args=(session, process_fields, req_flags))
            tr_req.start()
        else:
            self._threaded_node_request(session, process_fields, req_flags)
    ...
    def _handle_event_fields(self, msg):
        ...
        for node in msg['fields']['nodes']:
            for ts in node['timestamps']:
                ...
                callback(from_jid=msg['from'], result="fields",
                         nodeId=node['nodeId'], timestamp=ts['value'], fields=fields)
    ...
    def _handle_event_failure(self, msg):
        ...
        callback = self.sessions[seqnr]["callback"]
        callback(from_jid=msg['from'], result="failure", nodeId=msg['failure']['error']['nodeId'],
                 timestamp=msg['failure']['error']['timestamp'], error_msg=msg['failure']['error']['text'])
