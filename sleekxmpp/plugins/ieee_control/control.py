...
from sleekxmpp.plugins.ieee_control import stanza
from sleekxmpp.plugins.ieee_control.stanza import Control


log = logging.getLogger(__name__)


class IEEE_Control(BasePlugin):

    """
    IEEE_Control: IoT Control 
    ...
    name = 'ieee_control'
    description = 'IEEE Internet of Things - Control'
            ...
            for f in iq['set']['datas']:
                for node in self.nodes:
                    if not self.nodes[node]["device"].has_control_field(f['name'], f._get_typename()):
                        ...
                        break
            process_fields = [(f['name'], f._get_typename(), f['value'])
                              for f in iq['set']['datas']]
    ...
    def _threaded_node_request(self, session, process_fields):
        ...
        for node in self.sessions[session]["node_list"]:
            timer = Timer(self.nodes[node]['commTimeout'],
                          self._event_comm_timeout, args=(session, node))
    ...
    def set_request(self, from_jid, to_jid, callback, fields, nodeIds=None):
        ...
        self.sessions[seqnr] = {"from": iq['from'],
                                "to": iq['to'], "callback": callback}
        iq.send(block=False)
